        ...
        try
        {
            using (var framework = new EosFramework())
            {
                ...
                using (var camera = cameras[0])
                {
                    ...
                    camera.PictureTaken += (sender, e) =>
                    {
                        try
                        {
                            ...
                            using (MemoryStream ms = new MemoryStream())
                            {
                                ...
                                File.WriteAllBytes("test_event_capture.jpg", bytes);
                            }
                            captureDone = true;
                        }
                        catch (Exception ex)
                    ...
                    try
                    {
                        camera.SavePicturesToHost(Environment.CurrentDirectory, "test_prefix");
                    ...
                    // Let's print kEdsPropID_SaveTo value if we can
                    try
                    {
                        // 0x00000010 is kEdsPropID_SaveTo
